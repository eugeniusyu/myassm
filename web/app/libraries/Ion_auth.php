            ...
            if ($user) {
                ...
                if (!$this->config->item('use_ci_email', 'ion_auth')) {
                    ...
                    $parse_data = array(
                        ...
                        'logo' => $this->Settings->site_name
                    );
                    $msg = read_file('./themes/' . $this->theme . 'auth/email/forgot_password.html');
        ...
        if (!$email_activation || $active == '1') {
            ...
            if ($id !== FALSE) {
                if ($notify) {
                    ...
                    $msg = read_file('./themes/' . $this->theme . 'auth/email/credentials.html');
                    $message = $this->parser->parse_string($msg, $parse_data);
			...
			$msg = read_file('./themes/' . $this->theme . 'auth/email/activate_email.html');
  			$message = $this->parser->parse_string($msg, $parse_data);
